                ...
                <div class="row">
                    <div class="col-md-12"> <br>
                        ...
                        <table id="ITTable" class="display table table-striped nowrap table-hover table-responsive table-bordered table-dark">
                            ...
                            <thead align="center">
                                <tr>
                                    <th width="10%">หมายเลขลูกค้า</th>
                                    <th width="20%">ชื่อผู้ใช้</th>
                                    <th width="20%">รหัส</th>
                                    <th width="20%">รหัสไปรษณี</th>
                                    <th width="20%">เขต</th>
                                    <th width="20%">ตำบล</th>
                                    <th width="20%">บ้านเลขที่</th>
                            ...
                            <tbody align="center">
                                ...
                                foreach ($result as $r) { ?>
                                    <tr>
                                        ...
                                        <td><?= $r['House_no'] ?></td>
                                        <td><?= $r['Province'] ?></td>
                                        <td><a href="updateform.php?Customer_ID=<?= $r['Customer_ID'] ?>&User_Name=<?= $r['User_Name'] ?>&Password=<?= $r['Password'] ?>&zipcode=<?= $r['zipcode'] ?>&District=<?= $r['District'] ?>&Sub_District=<?= $r['Sub_District'] ?>&House_no=<?= $r['House_no'] ?>&Province=<?= $r['Province'] ?>" class="btn btn-warning btn-sm">แก้ไข</a></td>
                                        <td><a href="delete.php?Customer_ID=<?= $r['Customer_ID'] ?>&User_Name=<?= $r['User_Name'] ?>&Password=<?= $r['Password'] ?>&zipcode=<?= $r['zipcode'] ?>&District=<?= $r['District'] ?>&Sub_District=<?= $r['Sub_District'] ?>&House_no=<?= $r['House_no'] ?>&Province=<?= $r['Province'] ?>" class=" btn btn-danger btn-sm " data-toggle=" modal" data-target="#ModalCenter" onclick="return confirm('ยืนยันการลบข้อมูล !!');">ลบ</a></td>
                                    </tr>
                                <?php }
                                ?>
                            </tbody>
